...
class TestGroupRoutes:
    ...
    def test_create_group_with_blank_fields(self):
        blank_name = self.valid_group.copy()
        blank_name["name"] = ""
        response = client.post("/groups", json=blank_name)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json()["detail"] == "Group name cannot be empty"

        blank_description = self.valid_group.copy()
        blank_description["description"] = ""
        response = client.post("/groups", json=blank_description)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json()["detail"] == "Group description cannot be empty"

        blank_owner_id = self.valid_group.copy()
        blank_owner_id["owner_id"] = ""
        response = client.post("/groups", json=blank_owner_id)
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json()["detail"] == "Group owner id cannot be empty"
    ...
    def test_create_group_with_invalid_owner_uuid(self):
        ...
        assert response.json()[
            "detail"] == "Group owner id must be a valid UUID version 4"
    ...
    def test_get_group_with_bad_uuid(self):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json()[
            "detail"] == "Group id must be a valid UUID version 4"
    ...
    def test_get_user_groups_bad_uuid(self):
        ...
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.json()[
            "detail"] == "User id must be a valid UUID version 4"
    ...
    def test_get_group_members_success(self, monkeypatch):
        ...
        def mock_get_group_members(self, group_id):
            return {"data": [{"user_id": "user1", "group_id": group_id, "created_at": "2023-01-01T00:00:00"}]}
